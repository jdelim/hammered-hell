...
public class BossAI : MonoBehaviour
{
    ...
    public float runSpeed = 5f; //movement speed for when the player is detected
    ...
    void Start(){
        ...
        //bossAnimator = GetComponent<Animator>(); //gets the value of the animator from the object the script is attatched to 
    ...
    private void LateUpdate()//locks the x rotation
    {
        // Get the current rotation of the object
        Vector3 rotation = transform.localEulerAngles;

        // Set the x rotation to a fixed value (e.g. 0)
        rotation.x = 0;
    ...
    public void FollowPlayer()
    {
        ...
        //bossAnimator.SetBool("walk", true);
        agent.speed = runSpeed; //makes the enemy move master to "chase" the player
    ...
    if (Vector3.Distance(transform.position, player.position) <= attackDistance)
    {
        ...
        if (Time.time >= nextAttackTime)
        {
            ...
            //bossAnimator.SetBool("attak1", true);
        ...
        //bossAnimator.SetBool("attak1", false);
        }
